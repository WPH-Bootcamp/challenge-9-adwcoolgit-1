...
function ReviewAvatar({
  ...
  return (
    <div className='flex size-14.5 items-center justify-center rounded-full bg-[linear-gradient(135deg,#fdb022_0%,#c12116_100%)] text-base font-bold text-white'>
      {initials || 'JD'}
    </div>
  );
}
    ...
    <article className='flex flex-col gap-4 rounded-2xl bg-white p-4 shadow-[0_0_10px_rgba(203,202,202,0.25)]'>
      <div className='flex items-start gap-3'>
        ...
        <div className='min-w-0 text-neutral-950'>
          <p className='truncate text-md font-extrabold leading-7.5 tracking-tight'>
            {review.authorName}
          </p>
          <p className='text-sm md:text-base font-normal leading-7 tracking-tight text-neutral-600'>
      ...
      <div className='flex flex-col gap-2'>
        <div className='flex items-center gap-0.5'>
          {Array.from({ length: 5 }, (_, index) => (
            <Star
              ...
              className={
                index < filledStars
                  ? 'size-6 fill-[#FDB022] text-[#FDB022]'
        ...
        <p className='text-sm md:text-base font-normal leading-7 tracking-tight text-neutral-950'>
          {review.comment ||
            'This diner left a positive impression without additional notes.'}
        </p>
      </div>
    </article>
  );
